...
import { getDatabaseTransactionCategories, saveDatabaseCategory } from '@/services/transaction-service';
...
export default function PlanRoute() {
  ...
  return <PlanScreen key={planView.period.id} planView={planView} categories={categories} onCategorySave={(category) => saveDatabaseCategory(database, category)} onPeriodStartDayChange={async (day) => {
    await setBudgetPeriodStartDay(database, day);
    await load();
  }} onPlanItemSave={async (item, draft) => {
    if (item && !('isAutomatic' in item && item.isAutomatic)) await updateDatabasePlanItem(database, item, draft);
    ...
    const wallet = [...planView.wallets].filter((candidate) => !candidate.archived).sort((left, right) => right.balance - left.balance)[0];
    if (!wallet) throw new Error('Belum ada Wallet aktif untuk pembayaran.');
    const now = new Date();
    await setDatabasePlanItemPaid(database, item, paid, wallet.id, `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`);
    await load();
