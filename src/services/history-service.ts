...
export function filterHistoryTransactions(
  ...
  return transactions.filter((transaction) => {
    ...
    if (walletId && transaction.walletId !== walletId && transaction.toWalletId !== walletId)
      return false;
    if (filter === 'all') return true;
    return (
      filter === transaction.type || (filter === 'expense' && transaction.type === 'adjustment')
    );
  });
}

export function filterHistoryPeriod(
  transactions: Transaction[],
  startDate?: string,
  endDate?: string,
) {
  return transactions.filter(
    (transaction) =>
      (!startDate || transaction.date >= startDate) && (!endDate || transaction.date <= endDate),
  );
}
