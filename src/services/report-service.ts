...
export async function getDatabaseReportView(database: SQLiteDatabase, rangeMonths = 3, today?: string): Promise<ReportView> {
  ...
  const netSavingByPeriod = [...periodRows].reverse().map((row) => { const item = toPeriod(row); const periodTotals = totalsFor(item); return { period: item, netSaving: periodTotals.income - periodTotals.expense - periodTotals.transferOut + periodTotals.transferIn }; });
  const snapshot: MockBudgetSnapshot = { totalIncome: totals.income, totalExpense: totals.expense, totalTransferIn: totals.transferIn, totalTransferOut: totals.transferOut, netSaving: totals.income - totals.expense - totals.transferOut + totals.transferIn, spareBudget: 0, availableBalance: 0, freeBalance: 0, goalBalance: 0, planItems: [] };
