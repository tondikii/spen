...
export async function getDatabaseSettings(database: SQLiteDatabase) {
  ...
  return { currency: currencyOptions.includes(row?.currency as CurrencyCode) ? row!.currency as CurrencyCode : 'IDR' as CurrencyCode, themeMode: row?.theme_mode === 'dark' ? 'dark' : 'light' };
}
