...
export async function completeSetup(database: SQLiteDatabase, wallets: SetupWalletDraft[], currency: CurrencyCode) {
  if (wallets.length === 0) throw new Error('Buat minimal satu Wallet');
  const drafts = wallets.map((wallet) => ({ name: wallet.name.trim(), initialBalance: wallet.initialBalance }));
  if (drafts.some((wallet) => !wallet.name)) throw new Error('Nama Wallet wajib diisi');
  if (drafts.some((wallet) => !Number.isSafeInteger(wallet.initialBalance))) throw new Error('Saldo awal harus berupa angka bulat');
  ...
  await database.withExclusiveTransactionAsync(async (transaction) => {
    ...
    const category = drafts.some((wallet) => wallet.initialBalance !== 0)
      ...
      : null;
    if (drafts.some((wallet) => wallet.initialBalance !== 0) && !category) throw new Error('Kategori Saldo Awal belum tersedia');
    for (const draft of drafts) {
      const wallet = await transaction.runAsync('INSERT INTO wallets (name, initial_balance, is_savings, archived) VALUES (?, 0, 0, 0);', draft.name);
      if (draft.initialBalance !== 0) {
        const now = new Date();
        await transaction.runAsync(
          `INSERT INTO transactions (type, wallet_id, category_id, amount, date, time, note, is_initial)
           VALUES (?, ?, ?, ?, ?, ?, ?, 1);`,
          draft.initialBalance > 0 ? 'income' : 'expense', wallet.lastInsertRowId, category!.id,
          Math.abs(draft.initialBalance), now.toISOString().slice(0, 10), now.toTimeString().slice(0, 5), 'Saldo awal Wallet',
        );
      }
    }
