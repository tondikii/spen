...
describe('database report service', () => {
  ...
  it('aggregates the active period, sorts expenses, and excludes transfers', async () => {
    ...
    await saveDatabaseTransaction(database, { type: 'income', walletId: source.id, toWalletId: null, categoryId: income, amount: 1000, date: '2026-10-02', time: '08:00', note: 'Gaji' });
    await saveDatabaseTransaction(database, { type: 'expense', walletId: source.id, toWalletId: null, categoryId: food, amount: 300, date: '2026-10-02', time: '09:00', note: 'Makan' });
    await saveDatabaseTransaction(database, { type: 'expense', walletId: source.id, toWalletId: null, categoryId: transport, amount: 100, date: '2026-10-02', time: '10:00', note: 'Transport' });
    await saveDatabaseTransaction(database, { type: 'transfer', walletId: source.id, toWalletId: destination.id, categoryId: null, amount: 250, date: '2026-10-03', time: '10:00', note: 'Tabung' });

    const view = await getDatabaseReportView(database, 3, '2026-10-10');
    expect(view.snapshot).toMatchObject({ totalIncome: 1000, totalExpense: 400, totalTransferIn: 250, totalTransferOut: 250, netSaving: 600 });
