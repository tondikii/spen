...
describe('history service', () => {
  it('filters by type, category, and wallet', () => {
    expect(
      filterHistoryTransactions(transactions, 'income', undefined, 'cash').map(({ id }) => id),
    ).toEqual(['income']);
    expect(
      filterHistoryTransactions(transactions, 'all', 'food', 'bank').map(({ id }) => id),
    ).toEqual(['food']);
  ...
  it('groups newest days first while preserving transaction order', () => {
    expect(groupHistoryByDate(transactions).map(({ date }) => date)).toEqual([
      '2026-09-02',
      '2026-09-01',
      '2026-08-31',
    ]);
