...
import { createGoal, archiveGoal, getDatabaseGoals, getGoalProgress, saveToGoal, updateGoal, withdrawFromGoal } from '@/services/goal-service';
...
describe('goal service', () => {
  ...
  it('creates and edits a Goal backed by a savings Wallet', async () => {
    ...
    const goal = await createGoal(database, { name: 'Dana Nikah', targetAmount: 5000, targetDate: '2027-01-01', walletId: wallet.id, monthlyContribution: 400 });

    expect(goal.archived).toBe(false);
    expect((await getWallets(database))[0].isSavings).toBe(true);
    expect(await getGoalProgress(database, goal)).toEqual({ goalId: goal.id, savedAmount: 1000, progressPercent: 20, achieved: false });

    const edited = await updateGoal(database, goal.id, { ...goal, name: 'Dana Nikah Baru', targetAmount: 1000 });
  ...
  it('saves through transfer and withdraws through an expense while preserving ledger consistency', async () => {
    ...
    const goal = await createGoal(database, { name: 'Dana Darurat', targetAmount: 5000, targetDate: null, walletId: goalWallet.id, monthlyContribution: 250 });
  ...
  it('archives a Goal without deleting its history', async () => {
    ...
    const goal = await createGoal(database, { name: 'Motor', targetAmount: 10000, targetDate: null, walletId: wallet.id, monthlyContribution: 0 });
