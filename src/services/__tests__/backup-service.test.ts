...
describe('backup service', () => {
  ...
  it('mengekspor seluruh data pengguna dan dapat memvalidasi payload', async () => {
    await database.runAsync("INSERT INTO wallets (name, initial_balance, is_savings, archived) VALUES ('Tunai', 100000, 0, 0);");
  ...
  it('menolak versi backup yang tidak cocok', () => {
    expect(() => parseBackupPayload(JSON.stringify({ version: 99, data: {} }))).toThrow(/Versi backup tidak didukung/);
  });

  it('restore menimpa data lama secara atomik', async () => {
    await database.runAsync("INSERT INTO wallets (name, initial_balance, is_savings, archived) VALUES ('Lama', 1, 0, 0);");
    const payload = await exportDatabase(database);
    payload.data.wallets[0].name = 'Hasil Restore';
    await database.runAsync("INSERT INTO wallets (name, initial_balance, is_savings, archived) VALUES ('Tambahan', 2, 0, 0);");

    await restoreDatabase(database, payload);

    expect(await database.getAllAsync<{ name: string }>('SELECT name FROM wallets ORDER BY id;')).toEqual([{ name: 'Hasil Restore' }]);
  });
});
