...
import { BottomTabInset, Layout, MaxContentWidth, Radius, Spacing, Typography } from '@/constants/theme';
...
function HistoryScreenContent({ transactions: transactionsProp, categories, wallets }: { transactions?: Transaction[]; categories?: Category[]; wallets?: Wallet[] } = {}) {
  ...
  const transactions = useMemo(() => getHistoryPage(page, 5, filterHistoryPeriod(transactionsProp ?? getHistoryPage(1, Number.MAX_SAFE_INTEGER), startDate, endDate)), [page, startDate, endDate, transactionsProp]);
  ...
  const chooseFilter = (key: HistoryChip) => { setFilter(key === 'makan' ? 'expense' : key); setCategoryId(key === 'makan' ? 'category-makan' : undefined); setWalletId(undefined); setPage(1); setFilterOpen(false); };
  const chooseWallet = (nextWalletId: string) => { setWalletId(nextWalletId); setCategoryId(undefined); setFilter('all'); setPage(1); setFilterOpen(false); };

  return <ThemedView style={styles.page}><SectionList
    sections={sections}
    keyExtractor={(item) => item.id}
    stickySectionHeadersEnabled
    onEndReached={() => setPage((current) => Math.min(current + 1, Math.max(1, Math.ceil((transactionsProp?.length ?? 5) / 5))))}
    ...
    ListEmptyComponent={<View style={styles.empty}><ThemedText style={[styles.emptyGlyph, { color: theme.pine }]}>◌</ThemedText><ThemedText type="subtitle">Belum ada catatan</ThemedText><ThemedText type="small" themeColor="muted">Tidak ada transaksi yang cocok dengan filter ini.</ThemedText></View>}
    ...
    renderItem={({ item }) => <HistoryTransaction transaction={item} categories={categories} wallets={wallets} />}
  ...
  <Modal transparent animationType="slide" visible={filterOpen} onRequestClose={() => setFilterOpen(false)}><Pressable style={[styles.overlay, { backgroundColor: theme.overlay }]} onPress={() => setFilterOpen(false)}><View style={[styles.filterSheet, { backgroundColor: theme.card }]}><ThemedText type="sectionHeading">Filter transaksi</ThemedText>{filterOptions.map((option) => <Pressable key={option.key} accessibilityRole="button" accessibilityLabel={`Pilih filter ${option.label}`} onPress={() => chooseFilter(option.key)} style={[styles.filterOption, { borderTopColor: theme.line }]}><ThemedText type="smallBold" themeColor={(option.key === 'makan' ? categoryId === 'category-makan' : categoryId === undefined && walletId === undefined && filter === option.key) ? 'pine' : 'ink'}>{option.label}</ThemedText></Pressable>)}{categories?.map((category) => <Pressable key={category.id} accessibilityRole="button" accessibilityLabel={`Pilih filter kategori ${category.name}`} onPress={() => { setCategoryId(category.id); setWalletId(undefined); setFilter(category.type === 'income' ? 'income' : category.type === 'transfer' ? 'transfer' : 'expense'); setPage(1); setFilterOpen(false); }} style={[styles.filterOption, { borderTopColor: theme.line }]}><ThemedText type="smallBold" themeColor={categoryId === category.id ? 'pine' : 'ink'}>{category.name}</ThemedText></Pressable>)}{wallets?.map((wallet) => <Pressable key={wallet.id} accessibilityRole="button" accessibilityLabel={`Pilih filter Wallet ${wallet.name}`} onPress={() => chooseWallet(wallet.id)} style={[styles.filterOption, { borderTopColor: theme.line }]}><ThemedText type="smallBold" themeColor={walletId === wallet.id ? 'pine' : 'ink'}>{wallet.name}</ThemedText></Pressable>)}</View></Pressable></Modal>
  </ThemedView>;
}
...
const styles = StyleSheet.create({
  page: { alignSelf: 'center', flex: 1, maxWidth: MaxContentWidth, width: '100%' }, safeArea: { flex: 1 }, content: { paddingBottom: BottomTabInset + 24, paddingHorizontal: Layout.pagePadding, paddingTop: 24 }, header: { alignItems: 'center', flexDirection: 'row', marginBottom: 20 }, back: { fontSize: 26, lineHeight: 30, padding: 6 }, headerCopy: { flex: 1, marginLeft: 14 }, eyebrow: { ...Typography.eyebrow }, title: { fontSize: 29, lineHeight: 32 }, sectionHeader: { borderBottomWidth: 1, flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 12 }, transaction: { alignItems: 'center', borderBottomWidth: 1, flexDirection: 'row', gap: 11, minHeight: 62, paddingVertical: 12 }, icon: { alignItems: 'center', borderRadius: 12, height: 35, justifyContent: 'center', width: 35 }, copy: { flex: 1, minWidth: 0 }, amount: { alignItems: 'flex-end', marginLeft: Spacing.two }, footer: { alignItems: 'center', padding: 22 }, filterBar: { alignItems: 'center', borderTopWidth: 1, bottom: 0, flexDirection: 'row', justifyContent: 'center', left: 0, paddingBottom: 12, paddingTop: 10, position: 'absolute', right: 0 }, chip: { borderRadius: Radius.pill, borderWidth: 1, paddingHorizontal: 11, paddingVertical: 8 }, overlay: { flex: 1, justifyContent: 'flex-end' }, filterSheet: { borderTopLeftRadius: 27, borderTopRightRadius: 27, gap: 4, padding: 21 }, filterOption: { borderTopWidth: 1, borderTopColor: '#E3E4DD', paddingVertical: 15 }, empty: { alignItems: 'center', gap: 8, paddingVertical: 58 }, emptyGlyph: { fontSize: 38 },
});
