...
export function TransactionForm({
  ...
  const resolvedInitialType: TransactionType =
    transaction?.type === "adjustment"
      ? "expense"
      : (transaction?.type ?? initialType ?? "income");
  ...
  const [amount, setAmount] = useState(
    transaction
      ? formatMoneyInput(transaction.amount)
      : formatMoneyInput(initialAmount),
  );
  const [adminFee, setAdminFee] = useState(
    formatMoneyInput(transaction?.adminFee ?? ""),
  );
  ...
  const submit = () => {
    if (!walletId || !numericAmount || (type !== "transfer" && !categoryId))
      return;
    onSave({
      type,
      walletId,
        ...
        >
              ...
              {lockedToWalletId ? (
                ...
                >
                  <ThemedText type="smallBold">
                    {wallets.find((wallet) => wallet.id === lockedToWalletId)
                      ?.name ?? "Wallet Goal"}
                  </ThemedText>
                ...
                {visibleCategories.map((category) => (
                  <Pressable
                    ...
                    style={[
                      ...
                      {
                        borderColor:
                          category.id === categoryId ? theme.pine : theme.line,
                        backgroundColor:
                    ...
                    {CATEGORY_ICON_CHOICES.map((icon) => (
                      <Pressable
                        ...
                        style={[
                          ...
                          {
                            borderColor:
                              icon === categoryIcon ? theme.pine : theme.line,
                            backgroundColor:
                              icon === categoryIcon
                                ? theme.mint
                                : theme.background,
          ...
          {overBudget && (
            ...
            >
              <ThemedText type="small" style={{ color: theme.expense }}>
                Perlahan ya — ini akan melebihi alokasi, tetapi tetap bisa
                dicatat.
              </ThemedText>
            </View>
          )}
          {possibleDuplicate && (
            <View
              style={[
                styles.warning,
                { backgroundColor: theme.transferBackground },
              ]}
            >
          ...
          <Pressable
            ...
            style={[
              styles.walletChoice,
              {
                ...
                backgroundColor:
                  wallet.id === selected ? theme.mint : theme.card,
              },
            ]}
          >
