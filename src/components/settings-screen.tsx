...
export default function SettingsScreen({ database }: { database?: SQLiteDatabase } = {}) {
  ...
  return <ThemedView style={styles.page}><SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <ThemedView style={[styles.group, { backgroundColor: theme.card, borderColor: theme.line }]}><ActionRow icon="?" title="FAQ" detail="Jawaban singkat tentang Spen" onPress={() => setFaqOpen((open) => !open)} />{faqOpen && <View style={[styles.faq, { borderTopColor: theme.line }]}><FaqItem question="Apa itu Budget plan?" answer="Rencana global untuk Pendapatan, Pengeluaran, dan Goal dalam satu Budget period." theme={theme} /><FaqItem question="Apakah data tersimpan online?" answer="Tidak. Data Spen tersimpan lokal di perangkatmu." theme={theme} /><FaqItem question="Bagaimana cara mengoreksi saldo?" answer="Buka Wallet, pilih Koreksi saldo, lalu masukkan saldo riil terbaru." theme={theme} /></View>}<ActionRow icon="!" title="Aduan masalah" detail="Beri tahu kalau ada yang tidak beres" onPress={() => { void openLink('mailto:hello@spen.app?subject=Aduan%20masalah%20Spen'); }} /><ActionRow icon="›" title="Syarat & Ketentuan" detail="Baca ketentuan penggunaan" onPress={() => { void openLink('https://example.com/spen/terms'); }} /><ActionRow icon="›" title="Kebijakan Privasi" detail="Baca cara data dipakai" onPress={() => { void openLink('https://example.com/spen/privacy'); }} /></ThemedView>
...
function FaqItem({ question, answer, theme }: { question: string; answer: string; theme: ReturnType<typeof useTheme> }) { return <View style={[styles.faqItem, { borderBottomColor: theme.line }]}><ThemedText type="smallBold">{question}</ThemedText><ThemedText type="small" themeColor="muted" style={styles.faqAnswer}>{answer}</ThemedText></View>; }

const styles = StyleSheet.create({
  page: { flex: 1 }, safeArea: { flex: 1 }, content: { alignSelf: 'center', maxWidth: 430, paddingBottom: BottomTabInset + 24, paddingHorizontal: 21, paddingTop: 24, width: '100%' }, header: { marginBottom: 26, marginTop: 4 }, eyebrow: { ...Typography.eyebrow, marginBottom: 4 }, groupLabel: { marginBottom: 8, marginTop: 18 }, group: { borderRadius: 19, borderWidth: 1, overflow: 'hidden' }, row: { alignItems: 'center', borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: 'row', gap: 12, minHeight: 64, paddingHorizontal: 14, paddingVertical: 12 }, icon: { alignItems: 'center', borderRadius: 10, fontFamily: Fonts.mono, fontSize: 12, height: 31, justifyContent: 'center', textAlign: 'center', width: 31 }, copy: { flex: 1 }, rowTitle: { fontSize: 12, lineHeight: 16 }, rowDetail: { fontSize: 10, lineHeight: 14 }, chevron: { fontSize: 22, lineHeight: 26 }, toggle: { borderRadius: Radius.pill, height: 22, padding: 3, width: 37 }, toggleKnob: { backgroundColor: '#FFFFFF', borderRadius: Radius.pill, height: 16, width: 16 }, dropdown: { alignItems: 'center', borderRadius: 10, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 7 }, currencyGrid: { borderTopWidth: StyleSheet.hairlineWidth, flexDirection: 'row', flexWrap: 'wrap', gap: 8, padding: 13 }, currency: { alignItems: 'center', borderRadius: 10, borderWidth: 1, flexDirection: 'row', gap: 6, paddingHorizontal: 10, paddingVertical: 8 }, currencySymbol: { fontFamily: Fonts.mono, fontSize: 11 }, faq: { borderTopWidth: StyleSheet.hairlineWidth, paddingHorizontal: 14 }, faqItem: { borderBottomWidth: StyleSheet.hairlineWidth, gap: 3, paddingVertical: 12 }, faqAnswer: { lineHeight: 16 }, footer: { alignItems: 'center', gap: 4, marginTop: 53 }, toast: { alignSelf: 'center', borderRadius: 12, bottom: 12, paddingHorizontal: 14, paddingVertical: 11, position: 'absolute' }, pressed: { opacity: 0.7 },
});
