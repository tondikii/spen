...
function PlanScreenContent({ planView = getPlanView(), categories = [], onPeriodStartDayChange, onItemAction, onPlanItemSave, onPlanItemDelete, onGoalSave, onGoalArchive, onGoalSaveAction, onGoalWithdraw, aiInput, onSuggestionApply }: { planView?: PlanView; categories?: Category[]; onPeriodStartDayChange?: (day: number) => void | Promise<void>; onItemAction?: (item: BudgetPlanItem, amount: number) => void | Promise<void>; onPlanItemSave?: (item: BudgetPlanItem | null, draft: PlanItemDraft) => void | Promise<void>; onPlanItemDelete?: (item: BudgetPlanItem) => void | Promise<void>; onGoalSave?: (goal: Goal | null, draft: GoalDraft) => void | Promise<void>; onGoalArchive?: (goal: Goal) => void | Promise<void>; onGoalSaveAction?: (goal: Goal) => void | Promise<void>; onGoalWithdraw?: (goal: Goal, amount: number) => void | Promise<void>; aiInput?: BudgetAIInput; onSuggestionApply?: (suggestion: BudgetSuggestion) => void | Promise<void> }) {
  ...
  return <ThemedView style={styles.page}>{aiError && <ThemedText type="small" style={{ color: theme.expense, paddingHorizontal: 21, paddingTop: 8 }}>{aiError}</ThemedText>}<ScrollView contentContainerStyle={styles.content}>
    ...
    <PlanSection title="Pendapatan" action="+ Tambah" theme={theme} onAction={() => setPlanItemEditor({ type: 'income' })}>{plan.incomeItems.length ? plan.incomeItems.map((item) => <PlanItem key={item.id} item={item} category={categories.find((category) => category.id === item.categoryId)} state={itemState(item)} action="Catat" color={theme.income} theme={theme} onAction={handleItemAction} onEdit={() => setPlanItemEditor({ type: item.type, item })} onDelete={onPlanItemDelete} />) : <EmptyPlan message="Belum ada Pendapatan." />}</PlanSection>
    <PlanSection title="Fixed expense" action="+ Tambah" theme={theme} onAction={() => setPlanItemEditor({ type: 'fixedExpense' })}>{plan.fixedExpenseItems.length ? plan.fixedExpenseItems.map((item) => <PlanItem key={item.id} item={item} category={categories.find((category) => category.id === item.categoryId)} state={itemState(item)} action={itemState(item)?.paymentStatus?.kind === 'Lunas' ? '' : 'Bayar'} color={theme.expense} theme={theme} onAction={handleItemAction} onEdit={() => setPlanItemEditor({ type: item.type, item })} onDelete={onPlanItemDelete} />) : <EmptyPlan message="Belum ada Fixed expense." />}</PlanSection>
    ...
    <PlanSection title="Alokasi" action="+ Tambah" theme={theme} onAction={() => setPlanItemEditor({ type: 'allocation' })}>{plan.allocationItems.length ? plan.allocationItems.map((item) => <PlanItem key={item.id} item={item} category={categories.find((category) => category.id === item.categoryId)} state={itemState(item)} action="" color={itemState(item)?.overBudget ? theme.expense : theme.pine} theme={theme} onAction={handleItemAction} onEdit={() => setPlanItemEditor({ type: item.type, item })} onDelete={onPlanItemDelete} />) : <EmptyPlan message="Belum ada Alokasi." />}</PlanSection>
  </ScrollView>
...
const styles = StyleSheet.create({
  ...
  itemActions: { alignItems: 'center', flexDirection: 'row', flexWrap: 'wrap', gap: 14, justifyContent: 'flex-end', marginTop: 4 },
