...
function PlanItem({ item, category, state, action, color, theme, onAction, onPayment, onEdit, onDelete }: { item: BudgetPlanItem; category?: Category; state?: PlanItemState; action: string; color: string; theme: ReturnType<typeof useTheme>; onAction: (item: BudgetPlanItem) => void; onPayment?: (item: BudgetPlanItem, paid: boolean) => void | Promise<void>; onEdit: () => void; onDelete?: (item: BudgetPlanItem) => void | Promise<void> }) {
  ...
  const confirmDelete = () => Alert.alert('Hapus item plan?', `${item.name} dan targetnya akan dihapus dari Budget plan.`, [{ text: 'Batal', style: 'cancel' }, { text: 'Hapus', style: 'destructive', onPress: () => void onDelete?.(item) }]);
