...
type Confirmation = { title: string; message: string; confirmLabel: string; onConfirm: () => void | Promise<void> };

const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'];

function PlanScreenContent({ planView = getPlanView(), categories = [], onPeriodStartDayChange, onItemAction, onItemPayment, onPlanItemSave, onPlanItemDelete, onCategorySave, onGoalSave, onGoalArchive, onGoalSaveAction, onGoalWithdraw, aiInput, onSuggestionApply }: { planView?: PlanView; categories?: Category[]; onPeriodStartDayChange?: (day: number) => void | Promise<void>; onItemAction?: (item: BudgetPlanItem, amount: number) => void | Promise<void>; onItemPayment?: (item: BudgetPlanItem, paid: boolean) => void | Promise<void>; onPlanItemSave?: (item: BudgetPlanItem | null, draft: PlanItemDraft) => void | Promise<void>; onPlanItemDelete?: (item: BudgetPlanItem) => void | Promise<void>; onCategorySave?: (category: Category) => Category | Promise<Category>; onGoalSave?: (goal: Goal | null, draft: GoalDraft) => void | Promise<void>; onGoalArchive?: (goal: Goal) => void | Promise<void>; onGoalSaveAction?: (goal: Goal) => void | Promise<void>; onGoalWithdraw?: (goal: Goal, amount: number) => void | Promise<void>; aiInput?: BudgetAIInput; onSuggestionApply?: (suggestion: BudgetSuggestion) => void | Promise<void> }) {
  ...
  const handleItemAction = (item: BudgetPlanItem) => {
    ...
    const amount = item.type === 'expense'
  ...
  return <ThemedView style={styles.page}>{aiError && <ThemedText type="small" style={{ color: theme.expense, paddingHorizontal: 21, paddingTop: 8 }}>{aiError}</ThemedText>}<ScrollView contentContainerStyle={styles.content}>
    <View style={styles.header}><View><ThemedText type="title" style={styles.title}>Rencana</ThemedText><Pressable accessibilityRole="button" accessibilityLabel="Ubah Budget period" onPress={() => setPeriodOpen(true)} style={[styles.dropdown, { borderColor: theme.line }]}><ThemedText type="code" themeColor="muted" style={styles.periodLabel}>{periodLabel}</ThemedText></Pressable></View><Pressable accessibilityRole="button" accessibilityLabel="Saran AI" onPress={() => setAiOpen(true)} style={[styles.aiButton, { borderColor: theme.line, backgroundColor: theme.mint }]}><ThemedText type="smallBold" themeColor="pine" style={styles.aiButtonText}>✦ Saran AI</ThemedText></Pressable></View>
    ...
    <View style={[styles.spare, { backgroundColor: theme.spareBackground }]}><View style={styles.spareCopy}><ThemedText style={[styles.spareLabel, { color: theme.spareText }]}>Spare budget</ThemedText><ThemedText style={[styles.spareAmount, { color: theme.spareText }]}>{formatMoney(snapshot.spareBudget)}</ThemedText><ThemedText style={[styles.spareNote, { color: theme.spareText }]}>pendapatan − pengeluaran − goal</ThemedText></View><View style={[styles.calmRing, { borderColor: theme.pine, borderTopColor: theme.spareText }]}><ThemedText type="code" style={{ color: theme.spareText }}>{sparePercent(snapshot)}%</ThemedText></View></View>
    <PlanSection title="Pendapatan" action="+ Tambah" theme={theme} onAction={() => setPlanItemEditor({ type: 'income' })}>{plan.incomeItems.length ? plan.incomeItems.map((item) => <PlanItem key={item.id} item={item} category={categories.find((category) => category.id === item.categoryId)} state={itemState(item)} action={'isAutomatic' in item && item.isAutomatic ? '' : 'Catat'} color={theme.income} theme={theme} onAction={handleItemAction} onEdit={() => setPlanItemEditor({ type: item.type, item })} onDelete={onPlanItemDelete} />) : <EmptyPlan message="Belum ada Pendapatan." />}</PlanSection>
    <PlanSection title="Pengeluaran" action="+ Tambah" theme={theme} onAction={() => setPlanItemEditor({ type: 'expense' })}>{plan.expenseItems.length ? plan.expenseItems.map((item) => <PlanItem key={item.id} item={item} category={categories.find((category) => category.id === item.categoryId)} state={itemState(item)} action={expenseAction(item)} color={theme.expense} theme={theme} onAction={handleItemAction} onPayment={onItemPayment} onEdit={() => setPlanItemEditor({ type: item.type, item })} onDelete={onPlanItemDelete} />) : <EmptyPlan message="Belum ada Pengeluaran." />}</PlanSection>
  ...
  {planItemEditor && <PlanItemFormModal item={planItemEditor.item} type={planItemEditor.type} categories={categories} theme={theme} onCategorySave={onCategorySave} onClose={() => setPlanItemEditor(null)} onSave={async (draft) => { await onPlanItemSave?.(planItemEditor.item ?? null, draft); setPlanItemEditor(null); }} />}
...
function PlanItemFormModal({ item, type, categories: categoriesProp, theme, onCategorySave, onClose, onSave }: { item?: BudgetPlanItem; type: PlanItemType; categories: Category[]; theme: ReturnType<typeof useTheme>; onCategorySave?: (category: Category) => Category | Promise<Category>; onClose: () => void; onSave: (draft: PlanItemDraft) => void | Promise<void> }) {
  ...
  const saveCategory = async () => {
    ...
    if (!name) return;
    const draft: Category = { id: `category-draft-${Date.now()}`, name, type: type === 'income' ? 'income' : 'expense', icon: categoryIcon, archived: false, isAdjustment: false };
    const saved = onCategorySave ? await onCategorySave(draft) : draft;
    setCategories((current) => [...current, saved]); setCategoryId(saved.id); setCategoryEditorOpen(false); setCategoryName('');
  };
  const submit = () => {
    const targetAmount = type === 'income' ? 0 : parseMoneyInput(target);
    if (!categoryId || !Number.isSafeInteger(targetAmount) || (type === 'expense' && targetAmount <= 0)) return;
        ...
        <KeyboardAvoidingView style={styles.sheetKeyboard} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={0}>
          <View style={[styles.goalSheet, { backgroundColor: theme.card }]}>
            ...
            <ScrollView style={styles.sheetScroll} contentContainerStyle={styles.goalSheetContent} keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets keyboardDismissMode="interactive">
              <ThemedText type="small" themeColor="muted">{type === 'income' ? 'Pendapatan mengikuti transaksi yang sudah diterima.' : 'Target mengikuti total pembayaran dalam periode ini.'}</ThemedText>
              {type === 'expense' && <><ThemedText type="code" themeColor="muted" style={styles.fieldLabel}>TARGET NOMINAL</ThemedText><View style={styles.moneyInputRow}><CurrencyMark /><ThemedInput accessibilityLabel="Target item plan" keyboardType="numeric" placeholder="0" value={target} onChangeText={(value) => setTarget(formatMoneyInput(value))} style={[styles.goalInput, styles.moneyInput]} /></View></>}
              <View style={styles.categoryLabelRow}><ThemedText type="code" themeColor="muted" style={styles.fieldLabel}>KATEGORI</ThemedText><Pressable accessibilityRole="button" accessibilityLabel="Tambah kategori dari item plan" onPress={() => setCategoryEditorOpen(true)}><ThemedText type="smallBold" themeColor="pine">+ Tambah</ThemedText></Pressable></View>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.goalWallets}>{options.map((category) => <Pressable key={category.id} accessibilityRole="button" accessibilityLabel={`Kategori ${category.name}`} onPress={() => setCategoryId(category.id)} style={[styles.goalWallet, { borderColor: categoryId === category.id ? theme.pine : theme.line, backgroundColor: categoryId === category.id ? theme.mint : theme.card }]}><CategoryIcon name={category.icon} color={theme.pine} size={18} /><ThemedText type="smallBold">{category.name}</ThemedText></Pressable>)}</ScrollView>
              {options.length === 0 && <ThemedText type="small" style={{ color: theme.expense }}>Belum ada kategori yang sesuai.</ThemedText>}
              {categoryEditorOpen && <View style={[styles.inlineCategoryEditor, { borderColor: theme.line, backgroundColor: theme.background }]}><View style={styles.categoryEditorHeader}><ThemedText type="smallBold">Tambah kategori</ThemedText><Pressable accessibilityRole="button" accessibilityLabel="Batal edit kategori item plan" onPress={() => { setCategoryEditorOpen(false); setCategoryName(''); }} hitSlop={8}><ThemedText type="smallBold" style={{ color: theme.expense }}>Batal</ThemedText></Pressable></View><ThemedInput accessibilityLabel="Nama kategori baru dari item plan" placeholder="Nama kategori" value={categoryName} onChangeText={setCategoryName} style={styles.goalInput} /><ScrollView style={styles.planIconScroll} contentContainerStyle={styles.planIconGrid} nestedScrollEnabled showsVerticalScrollIndicator>{CATEGORY_ICON_CHOICES.map((icon) => <Pressable key={icon} accessibilityRole="button" accessibilityLabel={`Pilih ikon ${icon}`} onPress={() => setCategoryIcon(icon)} style={[styles.planIconChoice, { borderColor: icon === categoryIcon ? theme.pine : theme.line, backgroundColor: icon === categoryIcon ? theme.mint : theme.card }]}><CategoryIcon name={icon} color={theme.pine} size={18} /></Pressable>)}</ScrollView><Pressable accessibilityRole="button" accessibilityLabel="Simpan kategori item plan" disabled={!categoryName.trim()} onPress={() => void saveCategory()} style={({ pressed }) => [styles.planSaveCategory, { backgroundColor: categoryName.trim() ? theme.pine : theme.line, opacity: pressed ? 0.78 : 1 }]}><ThemedText type="smallBold" style={{ color: categoryName.trim() ? theme.heroText : theme.muted }}>Simpan kategori</ThemedText></Pressable></View>}
...
function GoalSection({ goals, wallets, theme, onSave, onArchive, onSaveAction, onWithdraw }: { goals: Goal[]; wallets: Wallet[]; theme: ReturnType<typeof useTheme>; onSave?: (goal: Goal | null, draft: GoalDraft) => void | Promise<void>; onArchive?: (goal: Goal) => void | Promise<void>; onSaveAction?: (goal: Goal) => void | Promise<void>; onWithdraw?: (goal: Goal, amount: number) => void | Promise<void> }) {
  ...
  return <>
    ...
    <ConfirmationModal visible={confirmation !== null} title={confirmation?.title ?? ''} message={confirmation?.message ?? ''} confirmLabel={confirmation?.confirmLabel ?? 'Konfirmasi'} destructive onCancel={() => setConfirmation(null)} onConfirm={async () => { const action = confirmation?.onConfirm; setConfirmation(null); await action?.(); }} />
  </>;
}
        ...
        <KeyboardAvoidingView style={styles.sheetKeyboard} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={0}>
          <View style={[styles.goalSheet, { backgroundColor: theme.card }]}>
            ...
            <ScrollView style={styles.sheetScroll} contentContainerStyle={styles.goalSheetContent} keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets keyboardDismissMode="interactive">
              ...
              <ThemedInput accessibilityLabel="Nama Goal" placeholder="Mis. Dana Nikah" value={name} onChangeText={setName} style={styles.goalInput} />
              <ThemedText type="code" themeColor="muted" style={styles.fieldLabel}>TARGET NOMINAL</ThemedText>
              <View style={styles.moneyInputRow}><CurrencyMark /><ThemedInput accessibilityLabel="Target Goal" keyboardType="numeric" placeholder="0" value={target} onChangeText={(value) => setTarget(formatMoneyInput(value))} style={[styles.goalInput, styles.moneyInput]} /></View>
              ...
              <ThemedInput accessibilityLabel="Tanggal target Goal" placeholder="YYYY-MM-DD" value={targetDate} onChangeText={setTargetDate} style={styles.goalInput} />
              <ThemedText type="code" themeColor="muted" style={styles.fieldLabel}>KONTRIBUSI BULANAN</ThemedText>
              <View style={styles.moneyInputRow}><CurrencyMark /><ThemedInput accessibilityLabel="Kontribusi bulanan Goal" keyboardType="numeric" placeholder="0" value={monthly} onChangeText={(value) => setMonthly(formatMoneyInput(value))} style={[styles.goalInput, styles.moneyInput]} /></View>
...
function PlanItem(props: { item: BudgetPlanItem; category?: Category; state?: PlanItemState; action: string; color: string; theme: ReturnType<typeof useTheme>; onAction: (item: BudgetPlanItem) => void; onPayment?: (item: BudgetPlanItem, paid: boolean) => void | Promise<void>; onEdit: () => void; onDelete?: (item: BudgetPlanItem) => void | Promise<void> }) {
  const [confirmation, setConfirmation] = useState(false);
  return <>
    <PlanItemContent {...props} onRequestDelete={() => setConfirmation(true)} />
    <ConfirmationModal visible={confirmation} title="Hapus item plan?" message={`${props.item.name} dan targetnya akan dihapus dari Budget plan.`} confirmLabel="Hapus" destructive onCancel={() => setConfirmation(false)} onConfirm={async () => { setConfirmation(false); await props.onDelete?.(props.item); }} />
  </>;
}

function PlanItemContent({ item, category, state, action, color, theme, onAction, onPayment, onEdit, onRequestDelete }: { item: BudgetPlanItem; category?: Category; state?: PlanItemState; action: string; color: string; theme: ReturnType<typeof useTheme>; onAction: (item: BudgetPlanItem) => void; onPayment?: (item: BudgetPlanItem, paid: boolean) => void | Promise<void>; onEdit: () => void; onRequestDelete: () => void }) {
  ...
  return <View style={[styles.item, { borderBottomColor: theme.line }]}><View style={styles.planItemTop}><View style={[styles.categoryIcon, { backgroundColor: income ? theme.incomeBackground : theme.expenseBackground }]}><CategoryIcon name={category?.icon} color={income ? theme.income : theme.expense} /></View><View style={styles.itemCopy}><ThemedText type="smallBold" style={styles.itemName}>{item.name}</ThemedText><ThemedText type="small" themeColor="muted" style={styles.itemSubtitle}>{income ? 'Dari transaksi' : paid ? 'Sudah dibayar' : 'Belum dibayar'}</ThemedText></View><ThemedText type="code" style={[styles.itemValueText, { color: income ? theme.income : theme.ink }]}>{income ? formatMoney(amount) : `${formatMoney(amount)} / ${formatMoney(item.targetAmount)}`}</ThemedText></View>{!income && <Progress value={progress} color={paid ? theme.income : color} theme={theme} />}<View style={styles.itemActions}>{!income && !item.isAutomatic && <Pressable accessibilityRole="switch" accessibilityLabel={`${paid ? 'Tandai belum dibayar' : 'Tandai sudah dibayar'} ${item.name}`} accessibilityState={{ checked: paid }} onPress={() => void onPayment?.(item, !paid)} hitSlop={8} style={styles.itemAction}><ThemedText type="smallBold" style={[styles.itemActionText, { color: paid ? theme.income : color }]}>{paid ? 'Sudah dibayar' : 'Belum dibayar'}</ThemedText></Pressable>}{action && !income && <Pressable accessibilityRole="button" accessibilityLabel={`${action} ${item.name}`} onPress={() => onAction(item)} hitSlop={8} style={styles.itemAction}><ThemedText type="smallBold" style={[styles.itemActionText, { color }]}>{action} →</ThemedText></Pressable>}<Pressable accessibilityRole="button" accessibilityLabel={`Edit ${item.name}`} onPress={onEdit} hitSlop={8} style={styles.itemAction}><ThemedText type="smallBold" themeColor="pine" style={styles.itemActionText}>Edit</ThemedText></Pressable>{!item.isAutomatic && <Pressable accessibilityRole="button" accessibilityLabel={`Hapus ${item.name}`} onPress={onRequestDelete} hitSlop={8} style={styles.itemAction}><ThemedText type="smallBold" style={[styles.itemActionText, { color: theme.expense }]}>Hapus</ThemedText></Pressable>}</View></View>;
}

function LegacyPlanItem({ item, category, state, action, color, theme, onAction, onEdit, onDelete }: { item: BudgetPlanItem; category?: Category; state?: PlanItemState; action: string; color: string; theme: ReturnType<typeof useTheme>; onAction: (item: BudgetPlanItem) => void; onEdit: () => void; onDelete?: (item: BudgetPlanItem) => void | Promise<void> }) {
  const progress = state?.progressPercent ?? 0;
  const amountLabel = state?.realizedAmount ?? 0;
  const subtitle = item.isAutomatic ? 'Dari transaksi' : item.type === 'income' ? 'Realisasi dari transaksi' : state?.overBudget ? 'Melebihi Budget' : 'Sisa budget';
  ...
  const displayAmount = item.targetAmount > 0 ? `${formatMoney(amountLabel)} / ${formatMoney(item.targetAmount)}` : formatMoney(amountLabel);
  const confirmDelete = () => undefined;
  if (item.type === 'income') return <View style={[styles.item, { borderBottomColor: theme.line }]}><View style={styles.planItemTop}><View style={[styles.categoryIcon, { backgroundColor: iconBackground }]}><ThemedText style={{ color: iconColor }}>{category?.icon ?? '◈'}</ThemedText></View><View style={styles.itemCopy}><ThemedText type="smallBold" style={styles.itemName}>{item.name}</ThemedText><ThemedText type="small" themeColor="muted" style={styles.itemSubtitle}>{item.isAutomatic ? 'Dari transaksi' : 'Realisasi dari transaksi'}</ThemedText></View><ThemedText type="code" style={[styles.itemValueText, { color: theme.income }]}>{formatMoney(amountLabel)}</ThemedText></View><View style={styles.itemActions}><Pressable accessibilityRole="button" accessibilityLabel={`Edit ${item.name}`} onPress={onEdit} hitSlop={8} style={styles.itemAction}><ThemedText type="smallBold" themeColor="pine" style={styles.itemActionText}>Edit</ThemedText></Pressable>{!item.isAutomatic && <Pressable accessibilityRole="button" accessibilityLabel={`Hapus ${item.name}`} onPress={confirmDelete} hitSlop={8} style={styles.itemAction}><ThemedText type="smallBold" style={[styles.itemActionText, { color: theme.expense }]}>Hapus</ThemedText></Pressable>}</View></View>;
  return <View style={[styles.item, { borderBottomColor: theme.line }]}><View style={styles.planItemTop}><View style={[styles.categoryIcon, { backgroundColor: iconBackground }]}><ThemedText style={{ color: iconColor }}>{category?.icon ?? '◈'}</ThemedText></View><View style={styles.itemCopy}><ThemedText type="smallBold" style={styles.itemName}>{item.name}</ThemedText><ThemedText type="small" themeColor="muted" style={[styles.itemSubtitle, state?.overBudget && { color: theme.expense }]}>{subtitle}</ThemedText></View><ThemedText type="code" style={[styles.itemValueText, { color: state?.overBudget ? theme.expense : theme.ink }]}>{formatMoney(amountLabel)} / {formatMoney(item.targetAmount)}</ThemedText></View><Progress value={progress} color={state?.overBudget ? theme.expense : color} theme={theme} /><View style={styles.itemActions}>{action && <Pressable accessibilityRole="button" accessibilityLabel={`${action} ${item.name}`} onPress={() => onAction(item)} hitSlop={8} style={styles.itemAction}><ThemedText type="smallBold" style={[styles.itemActionText, { color }]}>{action} →</ThemedText></Pressable>}<Pressable accessibilityRole="button" accessibilityLabel={`Edit ${item.name}`} onPress={onEdit} hitSlop={8} style={styles.itemAction}><ThemedText type="smallBold" themeColor="pine" style={styles.itemActionText}>Edit</ThemedText></Pressable><Pressable accessibilityRole="button" accessibilityLabel={`Hapus ${item.name}`} onPress={confirmDelete} hitSlop={8} style={styles.itemAction}><ThemedText type="smallBold" style={[styles.itemActionText, { color: theme.expense }]}>Hapus</ThemedText></Pressable></View></View>;
}
...
const styles = StyleSheet.create({
  page: { flex: 1 },
  safeArea: { flex: 1 },
  content: { alignSelf: 'center', maxWidth: 430, paddingBottom: 104, paddingHorizontal: 21, paddingTop: 24, width: '100%' },
  ...
  dropdown: { alignItems: 'center', borderRadius: 10, borderWidth: 1, paddingHorizontal: 9, paddingVertical: 6 },
  ...
  itemActions: { alignItems: 'center', flexDirection: 'row', flexWrap: 'wrap', gap: 16, justifyContent: 'flex-end', marginTop: 6 },
  ...
  categoryEditorHeader: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', minHeight: 28 },
  categoryLabelRow: { alignItems: 'baseline', flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 },
  ...
  inlineCategoryEditor: { alignSelf: 'stretch', borderRadius: 13, borderWidth: 1, gap: 8, marginTop: 4, padding: 11, width: '100%' },
  planIconScroll: { maxHeight: 196, width: '100%' },
  planIconGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 7, justifyContent: 'flex-start', width: '100%' },
  planIconChoice: { alignItems: 'center', borderRadius: 8, borderWidth: 1, height: 39, justifyContent: 'center', width: '18%' },
  planSaveCategory: { alignItems: 'center', borderRadius: Radius.medium, justifyContent: 'center', minHeight: 48, paddingHorizontal: 16, width: '100%' },
