UPDATE income_items SET budget_plan_id = (SELECT MIN(keeper.id) FROM budget_plans keeper WHERE keeper.budget_period_id = (SELECT duplicate.budget_period_id FROM budget_plans duplicate WHERE duplicate.id = income_items.budget_plan_id)) WHERE budget_plan_id IN (SELECT id FROM budget_plans WHERE id NOT IN (SELECT MIN(id) FROM budget_plans GROUP BY budget_period_id));--> statement-breakpoint
UPDATE fixed_expense_items SET budget_plan_id = (SELECT MIN(keeper.id) FROM budget_plans keeper WHERE keeper.budget_period_id = (SELECT duplicate.budget_period_id FROM budget_plans duplicate WHERE duplicate.id = fixed_expense_items.budget_plan_id)) WHERE budget_plan_id IN (SELECT id FROM budget_plans WHERE id NOT IN (SELECT MIN(id) FROM budget_plans GROUP BY budget_period_id));--> statement-breakpoint
