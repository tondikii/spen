...
**Budget plan**:
Rencana bulanan global (satu per aplikasi) yang menyatukan pendapatan, fixed expense, dan goal menjadi alokasi tabungan. Tidak terikat per wallet; wallet hanya tempat uang disimpan. Satu Budget plan aktif per Budget period.
...
**Goal**:
Target menabung pengguna (misal dana nikah, umroh, beli motor) dengan target biaya, jangka waktu, dan jumlah yang sudah terkumpul. Opsional; jangka waktu opsional (tanpa jangka waktu, kontribusi 0 dan tidak mempengaruhi spare budget). Goal adalah tracker — menabung untuk goal tidak otomatis mengurangi saldo wallet; uang untuk goal tetap di wallet. Saat target tercapai, ditandai "Tercapai" dan berhenti dihitung dalam spare budget. Dapat di-archive.
