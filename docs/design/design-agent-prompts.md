...
**Urutan yang disarankan:** Stitch (gratis, mobile-first, mockup cepat) → Figma Make (prototype fungsional, tapi kredit mahal). **Open Design** punya panduan sendiri di `docs/design/open-design-guide.md` (DESIGN.md + design-tokens.md + prompt per layar).
...
PERANMU: kamu design lead-nya. Aku kasih arah produk + arah tema; KEPUTUSAN VISUAL SPESIFIK
(font, warna exact, radius, spacing, style ikon) kamu yang tentukan, konsisten di semua layar.
Acuan tema: docs/design/DESIGN.md + docs/design/design-tokens.md (calm finance, angka menonjol,
semantic color income/expense/transfer, light + dark).
