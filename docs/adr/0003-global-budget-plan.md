...
Plan hanya memiliki Pendapatan dan Pengeluaran. Pendapatan adalah realisasi seluruh transaksi income (termasuk Saldo Awal dan Penyesuaian Saldo) tanpa progress. Pengeluaran unik berdasarkan kategori; expense kategori baru otomatis mendapat target awal sebesar realisasi (100%). Pembayaran dilakukan melalui toggle "Sudah dibayar".

Klarifikasi terbaru: Pendapatan boleh ditambahkan tanpa target nominal. Pembayaran Pengeluaran dilakukan melalui toggle "Sudah dibayar"; toggle aktif membuat satu transaksi expense sebesar target dari Wallet aktif dengan saldo terbesar, tanpa form transaksi, menggunakan tanggal saat toggle.
...
Revisi: Plan kini memiliki dua kelompok transaksi utama, **Pendapatan** dan **Pengeluaran**. Setiap transaksi income direalisasikan pada Pendapatan dan setiap transaksi expense pada Pengeluaran berdasarkan kategorinya. Konsep Fixed expense dan Alokasi digabung menjadi item `expense_items`; migrasi memindahkan item lama tanpa menghapus riwayat transaksi.
