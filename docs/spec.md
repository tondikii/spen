...
9. As a pengguna, I want mencatat pendapatan bulanan di Budget plan, so that saya tahu berapa uang masuk tiap periode.
...
15. As a pengguna, I want Goal bisa memiliki jumlah yang sudah terkumpul, so that progress menabung saya terlihat (misal goal 100jt, sudah 10jt = 10%).
...
26. As a pengguna, I want saldo wallet dihitung dari saldo awal + transaksi (tidak bisa diedit manual), so that saldo selalu konsisten dengan riwayat (koreksi lewat transaksi penyesuaian).
...
- **Wallet**: id, nama, saldo awal, archived. Wallet adalah tempat simpan; tidak punya plan sendiri; tidak punya tipe. **Saldo = saldo awal + transaksi** (tidak bisa diedit manual; koreksi lewat transaksi penyesuaian). Wallet yang sudah dipakai (punya transaksi) tidak bisa dihapus, hanya di-archive.
...
- **Budget plan**: global, satu per periode; berisi income (pendapatan), fixed expense items, dan goal contributions.
...
- **Goal**: id, nama, target jumlah, tanggal target (opsional), jumlah terkumpul, archived. Opsional; jangka waktu opsional (tanpa jangka waktu, kontribusi 0, tidak mempengaruhi spare). Saat tercapai, ditandai "Tercapai" dan berhenti dihitung dalam spare.
- **Transaction**: id, tipe (income/expense/transfer), wallet (untuk income/expense), kategori, jumlah, tanggal, catatan. **Edit transaksi = undo + redo** (kurangi wallet lama, tambah wallet baru) agar saldo konsisten.
...
- **Goal & saldo**: goal adalah tracker — menabung untuk goal tidak otomatis mengurangi saldo wallet; uang untuk goal tetap di wallet (user bisa mencatat transaksi atau menganggapnya bagian dari saldo). Kontribusi bulanan = (target − terkumpul) ÷ bulan tersisa; tanpa jangka waktu → kontribusi 0.
