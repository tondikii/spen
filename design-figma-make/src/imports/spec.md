...
26. As a pengguna, I want saldo wallet dihitung dari saldo awal + transaksi, dan saya bisa mengubah saldo langsung di UI yang otomatis membuat transaksi penyesuaian (kategori "Penyesuaian Saldo"), so that saldo tetap konsisten dengan riwayat dan saya bisa mengoreksi saldo yang melenceng tanpa harus tahu ke mana uang pergi.
...
- **Wallet**: id, nama, saldo awal, archived, flag tabungan (bool, untuk wallet goal). Wallet adalah tempat simpan; tidak punya plan sendiri; tidak punya tipe. **Saldo = saldo awal + transaksi** (tidak bisa diedit manual; koreksi lewat transaksi penyesuaian). **Edit saldo langsung di UI = shortcut yang otomatis membuat transaksi penyesuaian** (bukan mengubah saldo tanpa jejak). Wallet yang sudah dipakai (punya transaksi) tidak bisa dihapus, hanya di-archive.
...
- **Pendapatan (item plan)**: id, nama (misal "Gaji", "Freelance"), kategori income, nominal (target periode ini). **Realisasi = total transaksi income kategori tersebut dalam periode aktif**; tombol "Catat" membuat transaksi income (pilih wallet). Item pendapatan bisa lebih dari satu; masing-masing terikat satu kategori income. **Hapus item pendapatan = hapus juga transaksi income yang dibuat darinya** (undo+redo saldo), sehingga tidak ada realisasi yang menggantung.
...
- **Transaction**: id, tipe (income/expense/transfer), wallet (untuk income/expense), kategori, jumlah, tanggal, catatan. **Edit transaksi = undo + redo** (kurangi wallet lama, tambah wallet baru) agar saldo konsisten. **Koreksi saldo = transaksi penyesuaian** (kategori khusus "Penyesuaian Saldo", dibuat otomatis saat user mengubah saldo wallet langsung di UI); tidak mengisi progress item plan mana pun, tetapi mengurangi saldo wallet dan saldo tersedia.
